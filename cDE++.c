...
int a = 0, b = 0, count, encontrou = 0, countA = 0, i = 0, y = 0;
FILE *p;
...
int removerItem() {
    ...
    if (!encontrou) {
        ...
        if (a > b || a < 0) {
            ...
            fscanf("%s[^\n]", string);
            y = strcspn(string, '\n');
            fseek(p, +(y + 1), SEEK_CUR);
            fscanf("%s[^\n]", string);
            y = strcspn(string, '\n');
            fseek(p, +y, SEEK_CUR);                        
            fseek(p, -(numeroDeAlgarismos(b) + 1), SEEK_CUR);
            fprintf(p, "%d", b - a);
            countA = 0;
            printf("\nVocê removeu %d itens de %s do estoque com sucesso!", a, string);
        }
    }
